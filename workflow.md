...
- Use AI to sync `plan.md` with the final product behavior: Auto/Manual Voice, Chat-scoped background voice, bottom-sheet summary, History Continue context, and the `env.example` -> `env` -> `.env` configuration flow.
...
8. 提交前收口阶段：用 GPT-5.5/Codex 对照题目评分标准检查最终项目，包括可运行软件、移动端体验、清晰架构、取舍说明和必交文件；用 AI 将 `README.md` 收敛为适合评审的 Quick Start，包含一句话快速路径和详细步骤；用 AI 同步 `plan.md` 到最终产品行为，包括 Auto/Manual Voice、只在 Chat 生效的后台语音、summary 底部弹层、History Continue 上下文，以及 `env.example` -> `env` -> `.env` 配置链路；最后用本地 diff 和 `git diff --check` 作为文档修改的事实来源。
...
- Used AI to compare voice UX options and settle on Auto Voice as the default natural tutoring mode, with Manual Voice retained as a controlled fallback for demo reliability and transcript timing.
...
- 使用 AI 对比语音交互方案，并收敛为默认 Auto Voice 的自然口语练习模式，同时保留 Manual Voice 作为可控演示和 transcript 时序更明确的 fallback。
...
- iOS unit tests cover `SessionViewModel` connect/start/reconnect/end/transcript behavior with protocol mocks, History Continue message restoration, no-change review exits, same-record continued-practice updates after new input, Auto/Manual Voice mode behavior, Manual Voice buffered transcript display until send, active-Chat-only background scoping, `SessionStorageManager` latest-20 JSON persistence and clear-history, backend DTO decoding, summary display formatting, and specific failure-state mapping.
...
- AI Chat voice input now uses tap-to-record instead of hold-to-speak: the text field becomes a waveform while recording, the mic becomes `x` for cancel, send finishes voice input, and empty/no-speech voice input exits without adding an empty message.
- Chat now owns voice mode selection directly: long-pressing the mic button opens an above-input mode picker for `Auto Voice` and `Manual Voice`. `Auto Voice` is persisted as the default and can auto-start microphone publishing before background suspension only while Chat is active and connected.
...
- `plan.md` now reflects the final V1 behavior: Auto/Manual Voice, Chat-scoped background voice, bottom-sheet summary, keyboard-aware Chat input, History Continue bounded context, and aligned env template flow.
- Final physical-device validation is accepted as passed for the submission scope: Xcode install/run, microphone permission, microphone publishing, same-room agent join, one full realtime voice loop, learner/tutor transcript display, Auto/Manual Voice mode switching, background Auto Voice scoping, foreground waveform recovery, reconnect fallback, History Continue, local summary save, and optional AI summary update.
...
- iOS 单元测试覆盖 `SessionViewModel` 的 connect/start/reconnect/end/transcript 行为（使用协议 mock）、History Continue 消息恢复、只查看历史后退出不改变记录、有新输入后更新同一条继续练习记录、Auto/Manual Voice 模式行为、Manual Voice 发送前缓冲展示、后台语音只作用于活跃 Chat 的作用域、`SessionStorageManager` 最近 20 条 JSON 持久化和清空历史、后端 DTO 解码、summary 展示格式和具体失败状态映射。
...
- AI Chat 语音输入已从按住说话改为点击式录音：录音时文本输入框变成音波，麦克风变成 `x` 用于取消，发送按钮用于结束语音输入；没有捕获到语音时只退出语音模式，不添加空消息。
- Chat 现在直接承载语音模式选择：长按麦克风按钮会在输入栏上方弹出 `Auto Voice` / `Manual Voice` 选择。`Auto Voice` 作为默认模式持久化；只有当前 Chat 活跃且已连接时，才会在后台挂起前自动发布麦克风。
...
- `plan.md` 已同步最终 V1 行为：Auto/Manual Voice、只在 Chat 生效的后台语音、summary 底部弹层、跟随键盘的 Chat 输入区、History Continue 有限上下文，以及一致的 env 模板链路。
- 提交范围内的最终真机验证按已通过处理：包括 Xcode 安装/运行、麦克风权限、麦克风发布、agent 加入同一 room、一次完整实时语音闭环、学习者/tutor 转写展示、Auto/Manual Voice 切换、后台 Auto Voice 作用域、回前台音波恢复、重连 fallback、History Continue、本地 summary 保存和可选 AI summary 更新。
...
- If background audio fails, verify `UIBackgroundModes=audio`, active Chat scope, microphone publishing before backgrounding, `[test] Scene will resign active`, `[test] Scene entered background`, and whether iOS interrupted the audio session.
- If AI summary does not update, verify `/summary` API logs, transcript availability, and `SUMMARY_LLM_*` environment settings.
- If tap-to-record fails with a generic audio engine error, use Diagnostics and `[test]` logs to separate microphone permission, `AVAudioSession` route/category/mode, and LiveKit microphone publishing errors.
...
- 如果点击式语音输入只显示笼统的 audio engine error，使用 Diagnostics 和 `[test]` 日志区分麦克风权限、`AVAudioSession` 路由/category/mode 和 LiveKit 麦克风发布问题；
...
- Background audio is scoped to the active Chat audio session and does not claim unlimited background execution.
...
- 后台音频能力限定在活跃 Chat 音频会话内，不声明无限后台执行。
