...
- Backend local startup scripts exist for setup, API server, agent dev mode, and combined startup; root `start_all.sh` is the reviewer-facing entrypoint, stops stale local `uvicorn main:app` and `agent.py dev` processes before launch, clears and writes API/agent logs to `logs/`, waits for API health and agent `registered worker`, and scripts prefer the ignored root `.env` while using committed `env` only as a placeholder fallback.
...
- Documentation was updated for V1 product scope in `README.md`, `plan.md`, `todo.md`, and feature-level docs.
...
- 后端本地启动脚本已创建，覆盖 setup、API server、agent dev mode 和一键联合启动；根目录 `start_all.sh` 是评审入口，会在启动前停止旧的本地 `uvicorn main:app` 和 `agent.py dev` 进程，先清理再将 API/agent 日志写入 `logs/`，等待 API health 和 agent `registered worker`，脚本会优先使用被 git 忽略的根目录 `.env`，只有 `.env` 缺失时才使用已提交的 placeholder `env` 兜底；
...
- V1 产品范围已同步更新到 `README.md`、`plan.md`、`todo.md` 和功能级文档。
