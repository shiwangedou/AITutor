...
6. Agent chooses a startup-only voice profile from `VOICE_PIPELINE_PROFILE`.
7. `smooth` is the default demo profile and buffers each short TTS sentence before playback; `balanced` uses LiveKit's default streaming TTS path with very short replies; `realtime` uses LiveKit's default streaming TTS node with interruption enabled.
8. During a session, iOS can call `POST /summary/incremental` with the previous running summary and new transcript turns.
9. When iOS ends a session, it can call `POST /summary` with transcript text and the latest running summary.
10. Backend uses LiveKit Inference LLM to generate concise JSON summaries when available.
11. If AI summary generation fails, backend returns deterministic fallback summaries so local demo flow remains reliable.
12. Local demo startup is handled by scripts that prepare `.venv`, download model files, and run API + agent processes.
...
6. Agent 会在启动时根据 `VOICE_PIPELINE_PROFILE` 选择语音 profile。
7. `smooth` 是默认演示 profile，会在播放前缓冲完整短句 TTS；`balanced` 使用 LiveKit 默认流式 TTS 路径和极短回复；`realtime` 使用 LiveKit 默认流式 TTS 节点并允许打断。
8. 会话中，iOS 可以向 `POST /summary/incremental` 发送 previous running summary 和新增 transcript turns。
9. iOS 结束会话时，可以向 `POST /summary` 发送 transcript 文本和最新 running summary。
10. 后端优先使用 LiveKit Inference LLM 生成简短 JSON 摘要。
11. 如果 AI 摘要生成失败，后端返回确定性的 fallback 摘要，保证本地 demo 流程可靠。
12. 本地 demo 启动由脚本处理：准备 `.venv`、下载模型文件、运行 API 和 agent 进程。
